...
export default async function AdminPage() {
  ...
  return (
    <section className="shell grid gap-8 py-10">
      <h1 className="text-3xl font-black uppercase tracking-[0.1em] text-white">Admin Dashboard</h1>
      ...
      <div className="grid-auto">
        {["jobs", "users", "oracle", "activity", "actions"].map((page) => <Link className="panel p-6 text-xl font-bold capitalize text-cyan-300" key={page} href={`/admin/${page}`}>{page}</Link>)}
      </div>
    </section>
  );
}
